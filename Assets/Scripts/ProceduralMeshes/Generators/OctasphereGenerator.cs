...
namespace ProceduralMeshes.Generators
{
    ...
    public struct OctasphereGenerator : IMeshGenerator
    {
        ...
        public MeshJobScheduleDelegate DefaultJobHandle => MeshJob<OctasphereGenerator, MultiStream>.ScheduleParallel;
        ...
        private static Rhombus GetRhombus(int id) => id switch
        {
            ...
            _ => throw new System.Exception("Rhombus has 4 sides indexed from 0 to 3")
        };

        private static float2 GetTangentXZ(float3 p) => normalize(float2(-p.z, p.x));

        private float2 GetTextCoord(float3 p)
        {
            var texCoord = float2(
